...
$().ready(function () {
	...
	$.ajax({
					...
					for (lotNum in data[cemetery][blockNum]) {
						...
						for (graveNum in data[cemetery][blockNum][lotNum]) {
							for (g in data[cemetery][blockNum][lotNum][graveNum]) {
								...
								if (!birthYears.has(record.birthYear)) {
									birthYears.set(record.birthYear, []);
								}

								if (!deathYears.has(record.deathYear)) {
									deathYears.set(record.deathYear, []);
								}
								...
								birthYears.get(record.birthYear).push(recordID);
								deathYears.get(record.deathYear).push(recordID);
							}
						}
		...
		let nameMaps = [firstNames, middleNames, lastNames, maidenNames, otherInfoNames]
	...
	// }
		...
		function getMatches(fName_input, lName_input, sortOption, dobInput, dodInput, matchType) {
			...
			if (fName_input != "") {
				...
				for (const key of firstNames.keys()) {
					if (key.toLowerCase().includes(fName_input.toLowerCase())) {

						console.log(key, fName_input.toLowerCase(), key== fName_input.toLowerCase(), key.toLowerCase() != fName_input.toLowerCase(), matchType)

						if ((matchType == "exact" && key.toLowerCase() == fName_input.toLowerCase()) || (matchType == "partial" && key.toLowerCase().includes(fName_input.toLowerCase()))) { 
							firstNames.get(key).forEach(person => {
								if (fNameMatches.includes(person) == false) {
									fNameMatches.push(person)
								}

								if (partialNameMatches.includes(person) == false) {
									partialNameMatches.push(person)
								}

								if (anyMatches.includes(person) == false) {
									anyMatches.push(person)
								}
							})
						}
			...
			if (lName_input != "") {
				...
				for (const key of lastNames.keys()) {
					if (key.toLowerCase().includes(lName_input.toLowerCase())) {

						console.log(key, lName_input.toLowerCase(), key== lName_input.toLowerCase(), key.toLowerCase() != lName_input.toLowerCase(), matchType)

						if ((matchType == "exact" && key.toLowerCase() == lName_input.toLowerCase()) || (matchType == "partial" && key.toLowerCase().includes(lName_input.toLowerCase()))) { 
							lastNames.get(key).forEach(person => {
								if (lNameMatches.includes(person) == false) {
									lNameMatches.push(person)
								}
			...
			if (dobInput != "") {
				...
				for (const key of birthYears.keys()) {
					// console.log(birthYears.get(key))
					if (key == dobInput) {

						console.log(key, dobInput, key == dobInput, key != dobInput, matchType)
						birthYears.get(key).forEach(person => {
							if (dobMatches.includes(person) == false) {
								dobMatches.push(person)
							}

							if (partialYearMatches.includes(person) == false) {
								partialYearMatches.push(person)
							}
			...
			if (dodInput != "") {
				...
				for (const key of deathYears.keys()) {
					// console.log(birthYears.get(key))
					if (key == dodInput) {
						console.log(key)
						// dodMatches.push(deathYears.get(key))
						
						deathYears.get(key).forEach(person => {
							if (dodMatches.includes(person) == false) {
								dodMatches.push(person)
							}
			...
			fNameMatches.forEach(person => {
				if (lNameMatches.includes(person) && exactNameMatches.includes(person) == false) {
					exactNameMatches.push(person)
				}
			})


			dobMatches.forEach(person => {
				if (dodMatches.includes(person) && exactYearMatches.includes(person) == false) {
					exactYearMatches.push(person)
				}
			})
			...
			// 	First Name Matches: (${fNameMatches.length})\n\n[${fNameMatches}]\n
			// 	Last Name Matches: (${lNameMatches.length})\n\n[${lNameMatches}]\n
			// 	Exact Name Matches: (${exactNameMatches.length})\n\n[${exactNameMatches}]\n
			// 	Date of Birth Matches: (${dobMatches.length})\n\n[${dobMatches}]\n
			// 	Date of Death Matches: (${dodMatches.length})\n\n[${dodMatches}]\n
			// 	Exact Year Matches: (${exactYearMatches.length})\n\n[${exactYearMatches}]\n
			// 	Partial Name Matches: (${partialNameMatches.length})\n\n[${partialNameMatches}]\n
			// 	Partial Year Matches: (${partialYearMatches.length})\n\n[${partialYearMatches}]\n
			// 	Any Matches: (${anyMatches.length})\n\n[${anyMatches}]\n
			...
			// Sort records by record ID in ascending order 
			records.forEach(person => {
				if (fNameMatches.includes(person.recordID)) { fNameOutput.push(person) }
				if (lNameMatches.includes(person.recordID)) { lNameOutput.push(person) }
				if (dobMatches.includes(person.recordID)) { dobOutput.push(person) }
				if (dodMatches.includes(person.recordID)) { dodOutput.push(person) }

				if (partialNameMatches.includes(person.recordID)) { partialNameOutput.push(person) }
				if (partialYearMatches.includes(person.recordID)) { partialYearOutput.push(person) }

				if (exactNameMatches.includes(person.recordID)) { exactNameOutput.push(person) }
				if (exactYearMatches.includes(person.recordID)) { exactYearOutput.push(person) }

				if (anyMatches.includes(person.recordID)) { anyOutput.push(person) }
			})

			console.log()

			function nameSorter (o) {
				o.sort((a,b) => {
			...
			// TODO: SORTING RECORDS BOOKMARK
			switch (sortOption) {
				case "name":
					sortArr = [fNameOutput, lNameOutput, dobOutput, dodOutput, partialNameOutput, partialYearOutput, exactNameOutput, exactYearOutput, anyOutput]

					sortArr.forEach(array => {
						nameSorter(array)
					})
				...
				case "dod_latest":
					fNameOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))
					lNameOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))
					dobOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))
					dodOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))

					partialNameOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))
					partialYearOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))

					exactNameOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))
					exactYearOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))

					anyOutput.sort((a, b) => (a.dod > b.dod) ? 1 : ((b.dod > a.dod) ? -1 : 0))
					break;
				...
				case "age_youngest":
					fNameOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))
					lNameOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))
					dobOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))
					dodOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))

					partialNameOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))
					partialYearOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))

					exactNameOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))
					exactYearOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))

					anyOutput.sort((a, b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))
					break;
			...
			function printOrganizer(arr, isHidden) {

				arr.forEach(result => {
					console.log(result[0].length)
					if (result[0].length > 0) {
						printPersonResult(result[0], result[1], fName_input, lName_input, result[2], result[3], isHidden)
						isHidden = true
					}
				})
			...
			// TODO: FINSISH LOGIC FOR Partial, Exact and Any	
			if (matchType == "partial") {
				

					// printPersonResult(exactNameOutput, "Exact Matches", fName_input, lName_input, "exactNameResult", isHidden)
					// printPersonResult(fNameOutput, "First Name", fName_input, lName_input, "fNameResult", isHidden)
					// printPersonResult(lNameOutput, "Last Name", fName_input, lName_input, "lNameResult", isHidden)
					// printPersonResult(partialNameOutput, "Partial Name", fName_input, lName_input, "partialNameResult", isHidden)
					// printPersonResult(exactYearOutput, "Exact Year(s)", fName_input, lName_input, "exactYearResult", isHidden)
					// printPersonResult(dobOutput, "Date of Birth", fName_input, lName_input, "exactNameResult", isHidden)
					// printPersonResult(dodOutput, "Date of Death", fName_input, lName_input, "exactNameResult", isHidden)
					// printPersonResult(partialYearOutput, "Partial Year(s)", fName_input, lName_input, "exactNameResult", isHidden)

					let arr = [[exactNameOutput, "Exact Name", "exactNameResult"], [fNameOutput, "First Name", "fNameResult", fName_input], [lNameOutput, "Last Name", "lNameResult", lName_input], [exactYearOutput, "Exact Year", "exactYearResult", `${dobInput} and ${dodInput}`], [dobOutput, "Date of Birth", "dobResult", dobInput], [dodOutput, "Date of Death", "dodResult", dodInput]]
					
					// let arr = [[exactNameOutput, "Exact Matches", "exactNameResult"], [fNameOutput, "First Name Matches", "fNameResult"], [lNameOutput, "Last Name Matches", "lNameResult"], [partialNameOutput, "Partial Name Matches", "partialNameResult"], [dobOutput, "Date of Birth Matches", "dobResult"], [dodOutput, "Date of Death Matches", "dodResult"],[exactYearOutput, "Exact Year Matches", "exactYearResult"], [partialYearOutput, "Partial Year Matches", "partialYearResult"]]
					
					printOrganizer(arr, false)

			}


			
			// fNameOutput.sort((a, b) => (a.lastName > b.lastName) ? 1 : ((b.lastName > a.lastName) ? -1 : 0))
			// console.log(fNameOutput)


			// printPersonResult(fNameOutput, "First Name Matches", fName_input, lName_input, 'fNameResult', false)
		}
	...
	function printPersonResult(results, messageTitle, fName = fName_input, lName = lName_input, id = 'defautlResult', displayName = '', hidden = false) {
		...
		if (displayPerson != "" && displayName != "") {
			...
			if (results.length != 0) {
				...
				$results.append(`<h1 class='resultMessage'>${messageTitle}: [ ${results.length} ${resultStr} ]<br> <span>${displayName}</h1>`);
	...
	function displayPeople(d, id = '', hidden = false) {
		...
		$(`#results #${id}`).last().append(`
			<div id="${fName_clean}${lName_clean}" class="result">
				...
				<h3 class="location">${cemeteryLocation} Lawn - <span>Block ${blockNum}, Lot ${lotNum} : Grave ${graveNum}</span></h3>
				
				<a href='mailto:
					?subject=Augusta ${cemeteryLocation} Lawn Cemetery: ${fName} ${lName}
					&body=AUGUSTA CEMETERY ASSOCIATION - INFORMATION REQUEST:
					%0d%0a%0d%0aHere is the information we have on record for ${fName} ${lName}, located in the ${cemeteryLocation} Lawn Cemetery in Block ${blockNum}, Lot ${lotNum} [${graveNum}]%0d%0a
	...
	$("form").submit(function () {
		...
		var matchOption = $("#matchSelect").val();
		...
		if ((fNameInput == "" && lNameInput == "") && (dobInput == "" && dodInput == "")) {
			...
			getMatches(fNameInput, lNameInput, sortOption, dobInput, dodInput, matchOption)
		}
	});
});
